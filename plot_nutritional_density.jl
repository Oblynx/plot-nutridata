...
using DataFrames, FileIO, TableIO, XLSX, PlutoUI, Chain, CategoricalArrays, Plots, Images, OffsetArrays, Base.Filesystem, Colors, ColorSchemes

# ╔═╡ 6d82b894-1961-11ec-1805-210a4e8f669f
md"""
### Plot nutritional density

First, upload the excel file with the dataset:
"""

# ╔═╡ b42cb987-a26d-4b17-b419-d8eb71d1a519
@bind excel_file FilePicker([MIME("application/*")])

# ╔═╡ 1146db77-8904-4509-9692-b1177b52fe2a
md"""Excel sheet: $(@bind excel_sheet TextField())"""

# ╔═╡ c14c17d4-8f0a-44ea-84c7-e7384f6cf84a
md"""
Read the data from the excel file and parse the data type of each column:
"""

# ╔═╡ 03b698c0-9309-4fd5-a1b0-16c2aeda5630
begin
	data= DataFrame(read_table(excel_file, excel_sheet))
	nrows= size(data,1)
	# Parse types -> {string, float}
	dtypes= data[1,:]|>Vector.|>typeof
	for (i,c) in enumerate(names(data))
		if dtypes[i] <: String
			data[!,c]= convert.(String, data[!,c])
			if unique(data[!,c])|>length < nrows/3
				data[!,c]= categorical(data[!,c])
			end
		elseif dtypes[i] <: Number
			data[!,c]= convert.(Float64, data[!,c])
		end
	end
	dtypes= Dict(names(data) .=> eltype.(eachcol(data)))
	data
end
...
Which column are the row labels?
$(@bind names_col Select(filter(t->t[2]<:String, dtypes)|>keys|>collect))

### Colors

This is the colormap.
Which column should it correspond to?
$(@bind color_col Select(filter(t->t[2]<:CategoricalValue, dtypes)|>keys|>collect))

Note: the colormap only parses the values {"Low", "Moderate", "High", "Very high"}.

First we define 4 colors that work well together to represent the 4 values;
...
begin
	colormap(x)= Dict([
		...
		"Moderate"=> 3,
		"Low"=> 4])[x]
	palette= cgrad(plot_colors, 4, alpha=.7, categorical = true)
end
...
The data should be displayed in a particular order. Which data column should we use for ordering?
$(@bind order_column Select(filter(t->t[2]<:Number, dtypes)|>keys|>collect))
"""

# ╔═╡ 79d5226a-f55b-45b4-aeac-38f905a6080c
md"""
This is the spreadsheet ordered by "$(order_column)"
"""

# ╔═╡ 64534e58-c86e-4bac-9bab-8375cc19f429
begin
	orderby= order_column|>Symbol
	data_sortg= sort(data, orderby)
end
...
The left graph plots the sorting column $(orderby).
Which should be the column of the right graph?
$(@bind plotcol2 Select(filter(t->t[2]<:Number, dtypes)|>keys|>collect))

The following few cells create the graph, which is displayed a bit after.
"""

# ╔═╡ 827f7806-0496-4b91-aca5-40e440106d2b
begin
	num_annot(x)= text(string(x), :left, 11)
	annotateOverhang!(data,xlim, rows)= begin
		nRows= length(rows)
	...
	legendFilepath= joinpath(tmpdir,"nutrientDensityLegend.png")
	overlaidFilepath= joinpath(tmpdir,"nutrient_density.png")
	md"Some auxiliary plotting functions defined here."
end
...
# ╔═╡ d6f7d9b3-7397-4d7d-84a2-863e0e5ed219
md"Now we need to overlay the legend on the combined plot. This is the final plot:"
...
# ╔═╡ 50547ee0-ceef-4d0a-8455-f8c43560fce9
